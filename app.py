...
def generate_appointment_pdf(report, appointment, doctor_note, diet_plan):
    ...
    apt_data = [
        ...
        ['Doctor', appointment[8] or 'N/A'],
        ['Status', appointment[9] or 'N/A'],
    ]
...
def diet_plan():
    ...
    return render_template('diet_plan.html', plan=plan, prakriti=prakriti, name=session['user_name'])
...
def update_appointment(appointment_id, status):
    ...
    if apt:
        ...
        if patient_email:
            send_appointment_email(patient_email, apt[2], status, apt[5], apt[6], apt[7])
            flash(f'Appointment {status}! Email notification sent. 📧', 'success')
        else:
